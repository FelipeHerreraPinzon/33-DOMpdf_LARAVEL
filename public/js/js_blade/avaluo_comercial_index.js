            ...
            if (option == "update") {
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        ...
                        console.log(response[0].asigna_visitador);
                        $("#estado_id").val(response[0].estado_visitador);
                        $("#modal-title").text("Actualizando avaluo : " + response[0].codigo);
                    },
        ...
        .on("show.bs.modal", function (e) {
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    $("#referente_nombre").val(response.avaluo.referente_nombre);
                    $("#solicitante_nombre").val(response.avaluo.solicitante_nombre);
                    $("#banco").val(response.avaluo.cliente_nombres + " " + response.avaluo.cliente_apellidos);
                    $("#visitador_nombre").val(response.avaluo.visitador_nombre);
                    $("#informe_fecha_visita").val(response.avaluo.fecha_visita);
                    ...
                    //$("#atendido_por").val(response.avaluo.contacto_nombres + " " + response.avaluo.contacto_apellidos);
                    ...
                    $("#detalle_topografia").val(response.avaluo.detalle_topografia);
                    $("#numero_pisos").val(response.avaluo.numero_pisos);
                    $("#numero_sotanos").val(response.avaluo.numero_sotanos);
                    $("#detalle_tipo_inmueble").val(response.avaluo.detalle_tipo_inmueble);
                    $("#detalle_uso_actual").val(response.avaluo.detalle_uso_actual);
                    $("#detalle_estado_construccion").val(response.avaluo.detalle_estado_construccion);
                    ...
                    $("#informe_dependencia_id").val(response.avaluo.dependencia_id);
                    ...
                    $("#comedores_detalle").val(response.avaluo.comedores_detalle);
                    $("#estudios").val(response.avaluo.estudios);
                    $("#estudios_area").val(response.avaluo.estudios_area);
                    $("#estudios_detalle").val(response.avaluo.estudios_detalle);
                    $("#banos_sociales").val(response.avaluo.banos_sociales);
                    $("#banos_sociales_area").val(response.avaluo.banos_sociales_area);
                    $("#banos_sociales_detalle").val(response.avaluo.banos_sociales_detalle);
                    $("#star_habitaciones").val(response.avaluo.star_habitaciones);
                    $("#star_habitaciones_area").val(response.avaluo.star_habitaciones_area);
                    $("#star_habitaciones_detalle").val(response.avaluo.star_habitaciones_detalle);
                    $("#habitaciones").val(response.avaluo.habitaciones);
                    $("#habitaciones_area").val(response.avaluo.habitaciones_area);
                    $("#habitaciones_detalle").val(response.avaluo.habitaciones_detalle);
                    $("#banos_privados").val(response.avaluo.banos_privados);
                    $("#banos_privados_area").val(response.avaluo.banos_privados_area);
                    $("#banos_privados_detalle").val(response.avaluo.banos_privados_detalle);
                    ...
                    $("#cuartos_servicio").val(response.avaluo.cuartos_servicio);
                    $("#cuartos_servicio_area").val(response.avaluo.cuartos_servicio_area);
                    $("#cuartos_servicio_detalle").val(response.avaluo.cuartos_servicio_detalle);
                    $("#banos_servicio").val(response.avaluo.banos_servicio);
                    $("#banos_servicio_area").val(response.avaluo.banos_servicio_area);
                    $("#banos_servicio_detalle").val(response.avaluo.banos_servicio_detalle);
                    $("#patios_ropas").val(response.avaluo.patios_ropas);
                    $("#patios_ropas_area").val(response.avaluo.patios_ropas_area);
                    $("#patios_ropas_detalle").val(response.avaluo.patios_ropas_detalle);
                    $("#terrazas").val(response.avaluo.terrazas);
                    $("#terrazas_area").val(response.avaluo.terrazas_area);
                    $("#terrazas_detalle").val(response.avaluo.terrazas_detalle);
                    $("#jardines").val(response.avaluo.jardines);
                    $("#jardines_area").val(response.avaluo.jardines_area);
                    $("#jardines_detalle").val(response.avaluo.jardines_detalle);
                    $("#balcones").val(response.avaluo.balcones);
                    $("#balcones_area").val(response.avaluo.balcones_area);
                    $("#balcones_detalle").val(response.avaluo.balcones_detalle);
                    $("#zonas_verdes").val(response.avaluo.zonas_verdes);
                    $("#zonas_verdes_area").val(response.avaluo.zonas_verdes_area);
                    $("#zonas_verdes_detalle").val(response.avaluo.zonas_verdes_detalle);


                    //validacion de propiedad horizontal para que se despliegue o no el formulario de llenado
                    if (response.avaluo.propiedad_horizontal_id != null || response.avaluo.propiedad_horizontal_id != "") {
                        //$("#section_propiedad_horizontal").hide();
                        $("#section_propiedad_horizontal").show();
                        $("#informe_propiedad_horizontal_id").val(response.avaluo.propiedad_horizontal_id)
                    } else {
                        $("#section_propiedad_horizontal").hide();

                    }

                    $("#conjunto_cerrado").val(response.avaluo.conjunto_cerrado);
                    $("#ubicacion_inmueble").val(response.avaluo.ubicacion_inmueble);
                    $("#numero_edificios").val(response.avaluo.numero_edificios);
                    $("#unidades_por_piso").val(response.avaluo.unidades_por_piso);
                    $("#total_unidades").val(response.avaluo.total_unidades);
                    $("#total_garajes").val(response.avaluo.total_garajes);
                    $("#total_garajes_comunales").val(response.avaluo.total_garajes_comunales);
                    $("#garajes_cubiertos").val(response.avaluo.garajes_cubiertos);
                    $("#garajes_descubiertos").val(response.avaluo.garajes_descubiertos);
                    $("#total_garajes_servidumbre_comunales").val(response.avaluo.total_garajes_servidumbre_comunales);
